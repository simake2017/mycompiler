...
// 想了解 SFINAE 运行期发生了什么，`grep "\[sfinae\]" 输出` 得到的每一行
// 都能在这里找到出处与上下文。
// =============================================================================
...
// ─────────────────────────────────────────────────────────────────────────────
// 静态成员定义。深度为 0 表示"当前不在任何直接上下文内"——
// 此时若有 SubstitutionFailure 被抛出，说明没有任何吸收点会接住它。
...
// 产生信号
// ─────────────────────────────────────────────────────────────────────────────
// 直接上下文信息只是"记录"，不是"判断"：不在直接上下文里抛软失败信号，
// 在有些场景下是合法的（例如 resolveType 对非依赖 decltype 的立即求值试探，
// 失败后要保留节点延迟处理 —— 那里本就没有候选集）。所以这里只如实标注。
...
// void_t 探测链路
// ─────────────────────────────────────────────────────────────────────────────
// 探测是 SFINAE 里最容易讲错的一环，所以日志刻意写细：
// 逐条报告"第几个探测条件通过了"，让"全有或全无"这个语义在日志里看得见 ——
// 当后续某条失败时，前面那些"已通过"的记录不会被回滚，正是要展示这一点。
...
// decltype 求值失败被降级
// ─────────────────────────────────────────────────────────────────────────────
// 求值操作数时内层抛的多半是普通 runtime_error（如"类没有这个成员"）。
// 在别处那是硬错误，但在 decltype 探测里它恰恰是"这个类型不满足要求"的信号，
// 必须降级成软失败。这一行日志记录的就是这次降级 ——
// 它是"硬错误 → 软失败"的唯一转换点，出问题时第一个该看的地方。

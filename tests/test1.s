...
# ═══════════════════════════════════════════════════════════
    ...
    movl 8(%rax), %eax    # load .width (offset 8)
    pushq %rax
    movq -8(%rbp), %rax    # load this
    movl 12(%rax), %eax    # load .height (offset 12)
    movq %rax, %rcx
    popq %rax
    imulq %rcx, %rax             # *
    leave
    ...
    pushq %rax
    movq -8(%rbp), %rax    # load this
    movl 8(%rax), %eax    # load .radius (offset 8)
    movq %rax, %rcx
    popq %rax
    imulq %rcx, %rax             # *
    pushq %rax
    ...
    movq (%rdi), %rax            # rax = obj._vptr (at offset 0)
    # ═══ Virtual Call Step (b): Load function address from vtable ═══
    movq 24(%rax), %rax       # rax = vtable[1] (offset 24)
    # ═══ Virtual Call Step (c): Jump to the real function ═══
    callq *%rax                  # indirect call via vtable
    ...
    movq $10, %rax           # int literal
    movl %eax, 8(%rcx)    # .width (offset 8)
    movq $20, %rax           # int literal
    # member assign: .height = ...
    movq -16(%rbp), %rax    # load rect
    movq %rax, %rcx                # object address
    movq $20, %rax           # int literal
    movl %eax, 12(%rcx)    # .height (offset 12)
    # var a = ...
    # binary expr
    # member access: .width
    movq -16(%rbp), %rax    # load rect
    movq 8(%rax), %rax    # .width (offset 8)
    pushq %rax
    # member access: .height
    movq -16(%rbp), %rax    # load rect
    movq 12(%rax), %rax    # .height (offset 12)
    movq %rax, %rcx
    popq %rax
    imulq %rcx, %rax             # *
    movq %rax, -24(%rbp)    # store to a

...
# ═══════════════════════════════════════════════════════════
    ...
    ret                           # 返回调用者（从栈上弹出返回地址）
        ...
        .globl D_getD             # 导出函数符号，使链接器可见
    D_getD:                       # 函数入口标签
    # Function: getD (params: 0)
    pushq %rbp                    # 保存调用者的帧基址到栈上
    movq %rsp, %rbp               # 建立新栈帧：rbp = rsp（此后用 rbp+偏移访问局部）
    subq $64, %rsp              # 预留局部变量栈空间（16B 对齐）
    movq %rdi, -8(%rbp)         # 保存 this 指针到栈槽
    ...
    # return expr
    movq -8(%rbp), %rax    # 加载 this 指针
    movl 24(%rax), %eax    # 读取字段 .b（偏移 +24 字节）
    leave                         # 恢复栈帧（movq %rbp,%rsp; popq %rbp）
    ret                           # 返回调用者（从栈上弹出返回地址）
...
_ZTV1D:                        # vtable 标签
    ...
    .quad D_getD   # vtable[1]: 虚函数 D_getD
